...
def listar_circuitos(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return get_circuitos(db, skip=skip, limit=limit)

@route_circuitos.get("/{circuito_id}", response_model=CircuitoResponse)
def obtener_circuito(circuito_id: int, db: Session = Depends(get_db)):
    return get_circuito(db, circuito_id)
